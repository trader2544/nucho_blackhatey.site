...
document.addEventListener('DOMContentLoaded', function() {
    ...
    viewButtons.forEach(button => {
        button.addEventListener('click', (e) => {
            ...
            if (appId) {
                window.location.href = `price.html#${appId}-pricing`;
            }
        });
    });
...
document.querySelectorAll('a[href^="#"]').forEach(anchor => {
    ...
    const initPricingModal = () => {
        ...
        if (!modal || !viewButtons.length || !closeButton) return;

        // Show modal function
        const showModal = (appId) => {
            const pricingContent = document.getElementById(`${appId}-pricing`);
            if (pricingContent) {
                ...
                document.querySelector(`[data-app="${appId}"]`)?.classList.add('active');
            }
        };
        ...
        // Handle URL hash for direct pricing access
        const handleHash = () => {
            const hash = window.location.hash.slice(1);
            if (hash) {
                const appId = hash.replace('-pricing', '');
                showModal(appId);
            }
        };
        ...
        viewButtons.forEach(button => {
            button.addEventListener('click', (e) => {
                ...
                if (appId) {
                    e.preventDefault();
                    showModal(appId);
                    window.location.hash = `${appId}-pricing`;
                }
            });
        });
